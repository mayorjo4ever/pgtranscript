...
//        }
        ...
        if($request->isMethod('post')){   # if($request->ajax()){
           ...
           if($validator->fails()){ // or use $validator->passes()
               return redirect()->back()->with('error_message',$validator->messages());
...
//               }
                     ...
                     return redirect()->back()->with('error_message',"Invalid login parameters");
...
//        }
    ...
    public function forgot_password(Request $request) {
        if($request->isMethod('post')){ // sleep(3); // ->ajax()
                $data  = $request->all();
                $rules = [
