            ...
            <?php # print_r($printout->toarray()); <pre></pre> ?>
             ...
             @if(!empty($cover_letter))                      
                <?php $memo_url = base64_encode($cover_letter->regno."|".$cover_letter->id); ?>
                 &nbsp; &nbsp; <a href="{{url('admin/print-phd-transcript/'.$memo_url)}}" target="_blank" class="btn btn-primary "> PRINT TRANSCRIPT  [ {{ $cover_letter->print_count }} ]</a>
                 @endif
            ...
            <tr>
                <td colspan="3">
                    @php $regno = base64_encode($report->regno);
                        ...
                        $phdurl = base64_encode($regno."|".$purpose."|".$request_id."|".$type);
                    @endphp
                    
                    <a target="_blank" class="btn btn-success btn-lg p-3" href="{{url('admin/schedule-phd-transcript/'.$phdurl)}}">  View Transcript </a>
                    &nbsp; &nbsp; 
...
<div class="text-warning  mb-3" style="font-size: 1.5rem">
        ...
        @php $regno = base64_encode($regno);
            $purpose = base64_encode("Request"); # convocation / request
            $request_id = base64_encode($request_id);
            $type = base64_encode($request_type); # student / official
            $phdurl = base64_encode($regno."|".$purpose."|".$request_id."|".$type);
        @endphp  
<a href="{{url('admin/schedule-phd-transcript/'.$phdurl)}}" target="_blank" class="btn btn-info btn-lg p-3">Create New </a>
@endif
